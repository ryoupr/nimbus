...
# Performance Test Runner for EC2 Connect v3.0
# Task 17: Final Integration and Performance Testing
# Requirements: 5.1, 5.2

set -e

echo "🚀 EC2 Connect v3.0 - Performance Test Suite"
echo "============================================="
echo ""
...
if [ ! -f "Cargo.toml" ]; then
    print_error "Please run this script from the ec2-connect-rust directory"
    exit 1
fi
...
if command -v valgrind >/dev/null 2>&1; then
    ...
    valgrind --tool=massif --massif-out-file="$RESULTS_DIR/massif_$TIMESTAMP.out" \
        ./target/release/ec2-connect --help >> "$MEMORY_OUTPUT" 2>&1 || true
...
if command -v time >/dev/null 2>&1; then
    /usr/bin/time -v ./target/release/ec2-connect --help >> "$CPU_OUTPUT" 2>&1 || true
...
---
*Generated by EC2 Connect Performance Test Suite*
EOF
