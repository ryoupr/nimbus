...
pub trait SsmAgentDiagnostics {
    ...
    async fn check_managed_instance_registration(
        &self,
        instance_id: &str,
    ...
    async fn validate_ssm_service_registration(
        &self,
        instance_id: &str,
    ) -> Result<DiagnosticResult>;

    /// Get enhanced agent version information
    async fn get_enhanced_agent_version_info(
        &self,
        instance_id: &str,
    ) -> Result<Option<EnhancedAgentVersionInfo>>;

    /// Get registration analysis
    async fn get_registration_analysis(
        &self,
        instance_id: &str,
    ) -> Result<Option<RegistrationAnalysis>>;

    /// Get agent health metrics
    async fn get_agent_health_metrics(
        &self,
        instance_id: &str,
    ) -> Result<Option<AgentHealthMetrics>>;

    /// Get SSM service registration status
    async fn get_ssm_service_registration(
        &self,
        instance_id: &str,
    ) -> Result<Option<SsmServiceRegistration>>;
}
...
impl SsmAgentDiagnostics for DefaultSsmAgentDiagnostics {
            ...
            .await
        {
            Ok(response) => {
                ...
                if let Some(instance_info) = instances.first() {
                    debug!(
                        "Instance is registered as managed instance: {}",
                        instance_id
                    );
                    ...
                    Ok(DiagnosticResult::success(
                        ...
                        format!(
                            "Instance {} is registered as managed instance with ping status: {}",
                    ...
                    warn!(
                        "Instance is not registered as managed instance: {}",
                        instance_id
                    );
                    Ok(DiagnosticResult::error(
                        "managed_instance_registration".to_string(),
                        format!(
                            "Instance {} is not registered as managed instance in SSM",
                            instance_id
                        ),
                        start_time.elapsed(),
                        Severity::Critical,
            ...
            Ok(response) => {
                ...
                if let Some(instance_info) = instances.first() {
                    let agent_info = SsmAgentInfo {
                        ...
                        last_ping_time: instance_info.last_ping_date_time().and_then(|dt| {
                            chrono::DateTime::from_timestamp(dt.secs(), dt.subsec_nanos())
                        }),
                        ping_status: instance_info.ping_status().map(|s| s.as_str().to_string()),
                        platform_type: instance_info
                            .platform_type()
                        ...
                        registration_date: instance_info.registration_date().and_then(|dt| {
                            chrono::DateTime::from_timestamp(dt.secs(), dt.subsec_nanos())
                        }),
                        resource_type: instance_info
                            .resource_type()
                    ...
                    debug!(
                        "Instance not found in SSM managed instances: {}",
                        instance_id
                    );
                    Ok(None)
                }
    ...
    async fn run_ssm_agent_diagnostics(&self, instance_id: &str) -> Result<Vec<DiagnosticResult>> {
        info!(
            "Running comprehensive SSM Agent diagnostics: {}",
            instance_id
        );
        let start_time = Instant::now();
        ...
        let registration_result = self
            .check_managed_instance_registration(instance_id)
            .await?;
        ...
        info!(
            "SSM Agent diagnostics completed in {:?}",
            start_time.elapsed()
        );
        Ok(results)
    }
        ...
        match self.get_enhanced_agent_version_info(instance_id).await {
            Ok(Some(version_info)) => {
                ...
                match version_info.update_urgency {
                    UpdateUrgency::Critical => {
                        error!(
                            "Critical SSM Agent update required: {}",
                    ...
                    UpdateUrgency::High => {
                        warn!(
                            "High priority SSM Agent update recommended: {}",
                            version_info.current_version
                        );
                        Ok(DiagnosticResult::warning(
                            "enhanced_agent_version".to_string(),
                    ...
                    UpdateUrgency::Medium => {
                        info!(
                            "SSM Agent update available: {}",
                            version_info.current_version
                        );
                        Ok(DiagnosticResult::warning(
                            "enhanced_agent_version".to_string(),
                            format!(
                                "SSM Agent update available. Current: {}, Latest: {}",
                                version_info.current_version,
                                version_info
                    ...
                    UpdateUrgency::Low => {
                        info!(
                            "Minor SSM Agent update available: {}",
                            version_info.current_version
                        );
                        Ok(DiagnosticResult::success(
                            "enhanced_agent_version".to_string(),
                            format!(
                                "SSM Agent version {} is acceptable (minor update available)",
                                version_info.current_version
                            ),
                            start_time.elapsed(),
                        )
                    ...
                    UpdateUrgency::None => {
                        debug!(
                            "SSM Agent version is up to date: {}",
                            version_info.current_version
                        );
                        Ok(DiagnosticResult::success(
                            "enhanced_agent_version".to_string(),
            ...
            Ok(None) => {
                ...
                Ok(DiagnosticResult::error(
                    "enhanced_agent_version".to_string(),
                    format!(
                        "Enhanced agent version information not available for {}",
                        instance_id
                    ),
                    start_time.elapsed(),
                    Severity::High,
        ...
        match self.get_registration_analysis(instance_id).await {
            Ok(Some(analysis)) => {
                ...
                if analysis.registration_quality_score >= 80.0 {
                    debug!(
                        "Registration quality is excellent: {:.1}%",
                        analysis.registration_quality_score
                    );
                    Ok(DiagnosticResult::success(
                        "registration_analysis".to_string(),
                        format!(
                            "Registration quality excellent: {:.1}% ({})",
                            analysis.registration_quality_score, analysis.registration_status
                        ),
                        start_time.elapsed(),
                    )
                    .with_details(details))
                    ...
                    warn!(
                        "Registration quality needs improvement: {:.1}%",
                        analysis.registration_quality_score
                    );
                    Ok(DiagnosticResult::warning(
                        "registration_analysis".to_string(),
                        format!(
                            "Registration quality needs improvement: {:.1}% ({} issues found)",
                            analysis.registration_quality_score,
                    ...
                    error!(
                        "Registration quality is poor: {:.1}%",
                        analysis.registration_quality_score
                    );
                    Ok(DiagnosticResult::error(
                        "registration_analysis".to_string(),
                        format!(
                            "Registration quality is poor: {:.1}% ({} critical issues)",
                            analysis.registration_quality_score,
                            analysis.registration_issues.len()
                        ),
                        start_time.elapsed(),
            ...
            Ok(Some(health_metrics)) => {
                ...
                match health_metrics.health_trend {
                    HealthTrend::Critical => {
                        error!(
                            "Agent health is critical: {:.1}%",
                            health_metrics.overall_health_score
                        );
                        Ok(DiagnosticResult::error(
                            "agent_health_score".to_string(),
                            format!(
                                "Agent health is critical: {:.1}% (Error rate: {:.1}%)",
                                health_metrics.overall_health_score, health_metrics.error_rate
                            ),
                            start_time.elapsed(),
                            Severity::Critical,
                        )
                    ...
                    HealthTrend::Stable => {
                        info!(
                            "Agent health is stable: {:.1}%",
                            health_metrics.overall_health_score
                        );
                        Ok(DiagnosticResult::success(
                            "agent_health_score".to_string(),
                            format!(
                                "Agent health is stable: {:.1}% (Uptime: {:.1}%)",
                                health_metrics.overall_health_score,
    ...
    async fn validate_ssm_service_registration(
        ...
        match self.get_ssm_service_registration(instance_id).await {
            Ok(Some(service_registration)) => {
                ...
                let _registration_percentage =
                    (registered_services as f64 / total_services as f64) * 100.0;

                if registered_services == total_services {
                    debug!(
                        "All SSM services are registered: {}/{}",
                    ...
                    warn!(
                        "Most SSM services are registered: {}/{}",
                        registered_services, total_services
                    );
                    Ok(DiagnosticResult::warning(
                        "ssm_service_registration".to_string(),
                        format!(
                            "Most SSM services are registered ({}/{}) - Missing: {}",
                            registered_services,
                            total_services,
                            service_issues.join(", ")
                        ),
                    ...
                    error!(
                        "Many SSM services are not registered: {}/{}",
                        registered_services, total_services
                    );
                    Ok(DiagnosticResult::error(
                        "ssm_service_registration".to_string(),
                        format!(
                            "Many SSM services are not registered ({}/{}) - Missing: {}",
                            registered_services,
                            total_services,
                            service_issues.join(", ")
                        ),
            ...
            Ok(None) => {
                error!(
                    "SSM service registration info not available: {}",
                    instance_id
                );
                Ok(DiagnosticResult::error(
                    "ssm_service_registration".to_string(),
                    format!(
                        "SSM service registration information not available for {}",
                        instance_id
                    ),
                    start_time.elapsed(),
                    Severity::Medium,
            ...
            Ok(Some(agent_info)) => {
                if let Some(current_version) = &agent_info.agent_version {
                    ...
                    let version_age_days =
                        if let Some(registration_date) = agent_info.registration_date {
                            let now = chrono::Utc::now();
    ...
    async fn get_registration_analysis(
        ...
        match self.get_agent_info(instance_id).await {
            Ok(Some(agent_info)) => {
                ...
                if !agent_info.is_registered {
                    recommendations
                        .push("Ensure the instance has proper IAM permissions for SSM".to_string());
                    recommendations.push("Verify SSM agent is installed and running".to_string());
                }

                if ping_status != PingStatus::Online {
                    recommendations
                        .push("Check network connectivity to AWS SSM endpoints".to_string());
                }
                ...
                let registration_process_stages = vec![
                    ...
                    RegistrationStage {
                        ...
                        status: if agent_info.is_registered {
                            "Success".to_string()
                        } else {
                            "Failed".to_string()
                        },
            ...
            Ok(None) => {
                debug!(
                    "Agent info not available for registration analysis: {}",
                    instance_id
                );
                Ok(None)
            }
    ...
    async fn get_agent_health_metrics(
        ...
        match self.get_agent_info(instance_id).await {
            Ok(Some(agent_info)) => {
                ...
                let performance_health = if time_since_ping.as_secs() < 300 {
                    // 5 minutes
                    95.0
                } else if time_since_ping.as_secs() < 3600 {
                    // 1 hour
                    70.0
                } else {
                    30.0
            ...
            Ok(None) => {
                debug!(
                    "Agent info not available for health metrics: {}",
                    instance_id
                );
                Ok(None)
            }
    ...
    async fn get_ssm_service_registration(
        ...
        let inventory = ServiceRegistrationStatus {
            ...
            last_registration_time: Some(chrono::Utc::now()),
            capabilities: vec![
                "AWS:Application".to_string(),
                "AWS:InstanceInformation".to_string(),
            ],
            configuration_issues: Vec::new(),
        };
...
mod tests {
    ...
    fn test_ping_status_conversion() {
        ...
        assert_eq!(
            PingStatus::from("Connection Lost"),
            PingStatus::ConnectionLost
        );
    ...
    fn test_time_since_ping_calculation() {
        ...
        assert!(duration_1hr.as_secs() >= 3590 && duration_1hr.as_secs() <= 3610);
        // ~1 hour
    }
    ...
    fn test_ping_severity_determination() {
        ...
        assert_eq!(
            DefaultSsmAgentDiagnostics::determine_ping_severity(
                &PingStatus::ConnectionLost,
                moderate
            ),
            Severity::Medium
    ...
    fn test_enhanced_version_info_serialization() {
        let version_info = EnhancedAgentVersionInfo {
            ...
            vulnerability_info: vec![VulnerabilityInfo {
        ...
        assert_eq!(
            version_info.vulnerability_info.len(),
            deserialized.vulnerability_info.len()
        );
    }
    ...
    fn test_registration_analysis_creation() {
        let analysis = RegistrationAnalysis {
            ...
            registration_process_stages: vec![RegistrationStage {
