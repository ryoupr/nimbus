...
pub trait NetworkDiagnostics {
    ...
    async fn diagnose_network_configuration(
        &self,
        instance_id: &str,
...
impl DefaultNetworkDiagnostics {
    ...
    async fn get_subnet_route_table(&self, subnet_id: &str) -> Result<RouteTableInfo> {
        ...
        Err(anyhow::anyhow!(
            "Route table not found for subnet {}",
            subnet_id
        ))
    }

    /// Build route table info from AWS response
    fn build_route_table_info(
        &self,
        route_table: &aws_sdk_ec2::types::RouteTable,
    ) -> Result<RouteTableInfo> {
        ...
        let vpc_id = route_table
            .vpc_id
            .as_ref()
            .context("VPC ID not found")?
            .clone();

        let subnet_associations: Vec<String> = route_table
            .associations()
            .iter()
        ...
        let routes: Vec<RouteInfo> = route_table
            .routes()
            .iter()
            .map(|route| RouteInfo {
...
impl NetworkDiagnostics for DefaultNetworkDiagnostics {
                    ...
                    Ok(route_table) => {
                        ...
                        let has_internet_route = route_table.routes.iter().any(|route| {
                            route.destination_cidr_block.as_deref() == Some("0.0.0.0/0")
                                && (route
                                    .gateway_id
                                    .as_ref()
                                    .is_some_and(|gw| gw.starts_with("igw-"))
                        ...
                        let has_vpc_endpoint_routes = route_table.routes.iter().any(|route| {
                            route
                                .gateway_id
                                .as_ref()
                                .is_some_and(|gw| gw.starts_with("vpce-"))
                        });
                        ...
                        if has_internet_route || has_vpc_endpoint_routes {
                            ...
                            Ok(DiagnosticResult::success(
                                ...
                                format!(
                                    "Subnet {} has routes to SSM services via {}",
                                    subnet_id, route_type
                                ),
                                start_time.elapsed(),
                            )
                            .with_details(details))
    ...
    async fn diagnose_network_configuration(
        ...
        info!(
            "Running comprehensive network diagnostics for instance: {}",
            instance_id
        );
        ...
        info!(
            "Network diagnostics completed for instance: {}",
            instance_id
        );
        Ok(results)
    }
                        ...
                        for endpoint in endpoints {
                            if let Some(policy_doc) = &endpoint.policy_document {
                                ...
                                if policy_doc.contains("Deny") {
                                    policy_issues.push(format!(
                                        "Endpoint {} has Deny statements in policy",
                                        endpoint.service_name
                                    ));
                                }

                                if policy_doc.contains("Condition") {
                                    policy_analysis.push(format!(
                                        "Endpoint {} has conditional access",
                                        endpoint.service_name
                                    ));
                                }
                                ...
                                for action in ssm_actions {
                                    if policy_doc.contains(action) {
                                        policy_analysis.push(format!(
                                            "Endpoint {} allows {}",
                                            endpoint.service_name, action
                                        ));
                                    }
                                }
                            } else {
                                policy_analysis.push(format!(
                                    "Endpoint {} has no custom policy (full access)",
                                    endpoint.service_name
                                ));
                            }
                        }
    ...
    async fn verify_route_table_associations(&self, instance_id: &str) -> Result<DiagnosticResult> {
        ...
        match self.get_instance_network_info(instance_id).await {
            Ok((vpc_id, subnet_id)) => {
                ...
                match self.get_subnet_route_table(&subnet_id).await {
                    Ok(route_table) => {
                        association_analysis.push(format!(
                            "Subnet {} uses route table {}",
                            subnet_id, route_table.route_table_id
                        ));
                        ...
                        match self.check_ssm_vpc_endpoints(&vpc_id).await {
                            Ok(endpoints) => {
                                for endpoint in endpoints {
                                    if endpoint
                                        .route_table_ids
                                        .contains(&route_table.route_table_id)
                                    {
                                        association_analysis.push(format!(
                                            "Route table associated with VPC endpoint {}",
                                            endpoint.service_name
                                        ));
                                        ...
                                        if endpoint.subnet_ids.contains(&subnet_id) {
                                            association_analysis.push(format!(
                                                "Subnet associated with VPC endpoint {}",
                                                endpoint.service_name
                                            ));
                                        } else {
                                            association_issues.push(format!(
                                                "Subnet not associated with VPC endpoint {}",
                                                endpoint.service_name
                                            ));
                                        }
                                    } else {
                            ...
                            Err(e) => {
                                association_issues
                                    .push(format!("Failed to check VPC endpoints: {}", e));
                            }
                        }
                        ...
                        if has_igw_route {
                            association_analysis
                                .push("Route table has internet gateway route".to_string());
                        }
                        ...
                        if has_nat_route {
                            association_analysis
                                .push("Route table has NAT gateway route".to_string());
                        }
                    }
                    Err(e) => {
        ...
        for endpoint in ssm_endpoints {
            match tokio::net::lookup_host(format!("{}:443", endpoint)).await {
                Ok(addresses) => {
                    ...
                    dns_results.push(format!(
                        "DNS resolution successful for {} ({} addresses)",
                        endpoint, addr_count
                    ));
                }
                Err(e) => {
                ...
                match self.get_subnet_network_acls(&subnet_id).await {
                    Ok(network_acls) => {
                        ...
                        for acl in &network_acls {
                            ...
                            if has_https_outbound {
                                acl_analysis.push(format!(
                                    "Network ACL {} allows HTTPS outbound",
                                    acl.network_acl_id
                                ));
                            } else {
                                acl_issues.push(format!(
                                    "Network ACL {} may block HTTPS outbound",
                                    acl.network_acl_id
                                ));
                            }

                            // Check for overly restrictive rules
                            let deny_rules_count = acl
                                .entries
                                .iter()
                                .filter(|entry| entry.rule_action_deny)
                                .count();
                            if deny_rules_count > 0 {
                                acl_analysis.push(format!(
                                    "Network ACL {} has {} deny rules",
                                    acl.network_acl_id, deny_rules_count
                                ));
                            }
                        }
