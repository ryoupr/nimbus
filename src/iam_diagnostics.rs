use crate::aws::{load_aws_config, AwsManager};
...
pub trait IamDiagnostics {
    ...
    /// 権限境界の確認
    async fn check_permissions_boundary(
        &self,
        instance_id: &str,
    ) -> anyhow::Result<DiagnosticResult>;

    /// 一時的認証情報の妥当性確認
    async fn validate_temporary_credentials(&self) -> anyhow::Result<DiagnosticResult>;

    /// 包括的なIAM診断
    async fn diagnose_iam_configuration(
        &self,
        instance_id: &str,
    ) -> anyhow::Result<Vec<DiagnosticResult>>;

    // Enhanced methods for Task 25.3 - Comprehensive prerequisite checking

    /// 詳細なIAM権限の個別確認（必要な権限の個別確認）
    async fn verify_individual_ssm_permissions(
        &self,
        instance_id: &str,
    ) -> anyhow::Result<Vec<DiagnosticResult>>;

    /// クロスアカウント権限の詳細検証
    async fn verify_cross_account_permissions(
        &self,
        instance_id: &str,
    ) -> anyhow::Result<DiagnosticResult>;

    /// リソースベース権限の詳細分析
    async fn analyze_resource_based_permissions(
        &self,
        instance_id: &str,
    ) -> anyhow::Result<DiagnosticResult>;

    /// 権限境界の影響分析
    async fn analyze_permissions_boundary_impact(
        &self,
        instance_id: &str,
    ) -> anyhow::Result<DiagnosticResult>;

    /// EC2必須権限の詳細確認
    async fn verify_ec2_required_permissions(
        &self,
        instance_id: &str,
    ) -> anyhow::Result<DiagnosticResult>;
}
...
impl DefaultIamDiagnostics {
        ...
        for policy in &iam_info.attached_policies {
            if policy.has_ssm_permissions {
                ...
                if policy.policy_type == PolicyType::AwsManaged {
                    if self
                        .check_aws_managed_policy_permission(&policy.policy_arn, permission)
                        .await?
                    {
                        return Ok(true);
                    }
                    ...
                    if self
                        .check_policy_document_permission(&policy.policy_arn, permission)
                        .await?
                    {
                        return Ok(true);
                    }
    ...
    async fn check_aws_managed_policy_permission(
        &self,
        policy_arn: &str,
        permission: &str,
    ) -> anyhow::Result<bool> {
        ...
        let policy_permissions = match policy_arn {
            ...
            _ => {
                ...
                return self
                    .check_policy_document_permission(policy_arn, permission)
                    .await;
            }
        };
    ...
    async fn check_policy_document_permission(
        &self,
        policy_arn: &str,
        permission: &str,
    ) -> anyhow::Result<bool> {
        match self.get_policy_version(policy_arn).await {
            Ok(policy_document) => {
                ...
                Ok(policy_document.contains(permission)
                    ...
                    || policy_document
                        .contains(&format!("{}:*", permission.split(':').next().unwrap_or(""))))
            }
            Err(_) => Ok(false),
        }
    }
    ...
    async fn analyze_role_trust_policy(&self, role_name: &str) -> anyhow::Result<String> {
        ...
        if let Some(role) = response.role() {
            if let Some(trust_policy) = role.assume_role_policy_document() {
                ...
                if decoded_policy.contains("arn:aws:iam::")
                    && !decoded_policy.contains("arn:aws:iam:::")
                {
    ...
    async fn check_kms_permissions_for_instance(
        &self,
        _instance_id: &str,
                ...
                if instance.instance_id.as_deref() == Some(instance_id) {
                    if let Some(iam_instance_profile) = &instance.iam_instance_profile {
                        ...
                        if let Some(arn) = profile_arn {
                            // ARNからプロファイル名を抽出
                            let profile_name =
                                arn.split('/').next_back().unwrap_or("unknown").to_string();
    ...
    async fn get_instance_profile_details(&self, profile_name: &str) -> anyhow::Result<IamInfo> {
        ...
        if let Some(role) = roles.first() {
            ...
            // ロールの詳細情報を取得
            self.populate_role_details(&mut iam_info, role.role_name())
                .await?;
        }
    ...
    async fn populate_role_details(
        &self,
        iam_info: &mut IamInfo,
        role_name: &str,
    ) -> anyhow::Result<()> {
        debug!("Getting role details: {}", role_name);

        // アタッチされたポリシーを取得
        let attached_policies_response = self
            .iam_client
        ...
        for policy in policies {
            if let (Some(policy_name), Some(policy_arn)) =
                (policy.policy_name(), policy.policy_arn())
            {
                let has_ssm_permissions = self
                    .check_policy_ssm_permissions(policy_arn)
                    .await
                    .unwrap_or(false);
        ...
        // インラインポリシーを取得
        let inline_policies_response = self
            .iam_client
            .list_role_policies()
            .role_name(role_name)
            .send()
        ...
        if let Some(role) = role_response.role() {
            if let Some(permissions_boundary) = role.permissions_boundary() {
                iam_info.permissions_boundary = permissions_boundary
    ...
    async fn check_policy_ssm_permissions(&self, policy_arn: &str) -> anyhow::Result<bool> {
        ...
        if policy_arn.contains(":aws:policy/") {
            ...
            let broad_policies = vec!["PowerUserAccess", "AdministratorAccess"];
    ...
    async fn get_policy_version(&self, policy_arn: &str) -> anyhow::Result<String> {
        ...
        let policy_version = version_response
            .policy_version()
    ...
    async fn get_instance_metadata_token_expiration(
        &self,
        ...
        if let Some(expiration_str) = creds_json.get("Expiration").and_then(|v| v.as_str()) {
            let expiration =
                chrono::DateTime::parse_from_rfc3339(expiration_str)?.with_timezone(&chrono::Utc);
...
impl IamDiagnostics for DefaultIamDiagnostics {
    async fn check_instance_profile(&self, instance_id: &str) -> anyhow::Result<DiagnosticResult> {
        ...
        match self.get_instance_iam_info(instance_id).await {
            ...
            Ok(None) => Ok(DiagnosticResult::error(
                "instance_profile".to_string(),
            ...
            Ok(Some(iam_info)) => {
                ...
                for policy in &iam_info.attached_policies {
                    if policy.has_ssm_permissions {
                        ...
                        permission_details.push(format!(
                            "Policy '{}' has SSM permissions",
                            policy.policy_name
                        ));
                    }
                }
                ...
                if !iam_info.inline_policies.is_empty() {
                    permission_details.push(format!(
                        "Found {} inline policies (manual review recommended)",
        ...
        match self.get_instance_iam_info(instance_id).await {
            Ok(Some(iam_info)) => {
                ...
                if let Some(boundary_arn) = &iam_info.permissions_boundary {
                    ...
                    let boundary_allows_ssm = self
                        .check_policy_ssm_permissions(boundary_arn)
                        .await
                        .unwrap_or(false);
                    ...
                    if boundary_allows_ssm {
                        ...
                        Ok(DiagnosticResult::warning(
                            "permissions_boundary".to_string(),
                            format!(
                                "Permissions boundary may restrict SSM access: {}",
                                boundary_arn
                            ),
                            start_time.elapsed(),
                            Severity::High,
                    ...
                    if let Some(expiration) = status.expiration {
                        ...
                        if time_until_expiry < chrono::Duration::minutes(15) {
                            Ok(DiagnosticResult::warning(
                                "temporary_credentials".to_string(),
                                format!(
                                    "Session token expires soon: {} minutes remaining",
    ...
    async fn diagnose_iam_configuration(
        ...
        // 8. 権限境界の影響分析
        results.push(
            self.analyze_permissions_boundary_impact(instance_id)
                .await?,
        );
    ...
    async fn verify_individual_ssm_permissions(
        ...
        let required_permissions = vec![
            (
                "ssm:UpdateInstanceInformation",
                "Required for instance registration with SSM",
            ),
            ("ssm:SendCommand", "Required for executing commands via SSM"),
            ("ssm:ListCommands", "Required for listing command history"),
            (
                "ssm:ListCommandInvocations",
                "Required for command execution status",
            ),
            (
                "ssm:DescribeInstanceInformation",
                "Required for instance status reporting",
            ),
            (
                "ssm:GetConnectionStatus",
                "Required for connection status monitoring",
            ),
            (
                "ssm:DescribeInstanceAssociationsStatus",
                "Required for association status",
            ),
            (
                "ec2messages:AcknowledgeMessage",
                "Required for EC2 message acknowledgment",
            ),
            (
                "ec2messages:DeleteMessage",
                "Required for EC2 message cleanup",
            ),
            (
                "ec2messages:FailMessage",
                "Required for EC2 message failure handling",
            ),
            (
                "ec2messages:GetEndpoint",
                "Required for EC2 message endpoint discovery",
            ),
            (
                "ec2messages:GetMessages",
                "Required for EC2 message retrieval",
            ),
            (
                "ec2messages:SendReply",
                "Required for EC2 message responses",
            ),
            (
                "ssmmessages:CreateControlChannel",
                "Required for SSM control channel",
            ),
            (
                "ssmmessages:CreateDataChannel",
                "Required for SSM data channel",
            ),
            (
                "ssmmessages:OpenControlChannel",
                "Required for opening SSM control channel",
            ),
            (
                "ssmmessages:OpenDataChannel",
                "Required for opening SSM data channel",
            ),
        ];
            ...
            Ok(Some(iam_info)) => {
                for (permission, description) in required_permissions {
                    ...
                    let has_permission = self
                        .check_specific_permission(&iam_info, permission)
                        .await
                        .unwrap_or(false);
    ...
    async fn verify_cross_account_permissions(
        ...
        match self.get_instance_iam_info(instance_id).await {
            Ok(Some(iam_info)) => {
                        ...
                        Ok(trust_analysis) => {
                            ...
                            if trust_analysis.contains("cross-account") {
                                cross_account_issues
                                    .push("Cross-account trust relationship detected".to_string());
                            }
                        }
                        Err(e) => {
                            cross_account_issues
                                .push(format!("Failed to analyze trust policy: {}", e));
                        }
                    }
                }

                // Check for external ID requirements
                let external_id_required = self
                    .check_external_id_requirement(&iam_info)
                    .await
                    .unwrap_or(false);

                if external_id_required {
                    cross_account_details
                        .push("External ID required for cross-account access".to_string());
                }
                ...
                if cross_account_issues.is_empty() {
                    ...
                    Ok(DiagnosticResult::warning(
                        "cross_account_permissions".to_string(),
                        format!(
                            "Cross-account permission considerations: {}",
                            cross_account_issues.join(", ")
                        ),
                        start_time.elapsed(),
                        Severity::Medium,
                    )
            ...
            Err(e) => Ok(DiagnosticResult::error(
                "cross_account_permissions".to_string(),
    ...
    async fn analyze_resource_based_permissions(
        ...
        if resource_issues.is_empty() {
            ...
            Ok(DiagnosticResult::warning(
                "resource_based_permissions".to_string(),
                format!(
                    "Resource-based permission issues: {}",
                ...
                if let Some(boundary_arn) = &iam_info.permissions_boundary {
                    ...
                    match self.get_policy_version(boundary_arn).await {
                        Ok(policy_document) => {
                            ...
                            let ssm_permissions_in_boundary = self
                                .analyze_ssm_permissions_in_policy(&policy_document)
                                .await;
                            boundary_analysis.push(format!(
                                "SSM permissions in boundary: {:?}",
                                ssm_permissions_in_boundary
                            ));

                            // Check for restrictive conditions
                            if policy_document.contains("DateLessThan")
                                || policy_document.contains("DateGreaterThan")
                            {
                                boundary_restrictions
                                    .push("Time-based restrictions detected".to_string());
                            }

                            if policy_document.contains("IpAddress")
                                || policy_document.contains("IpAddressIfExists")
                            {
                                boundary_restrictions
                                    .push("IP-based restrictions detected".to_string());
                            }

                            if policy_document.contains("RequestedRegion") {
                                boundary_restrictions
                                    .push("Region-based restrictions detected".to_string());
                            }
                        ...
                        Err(e) => {
                            boundary_analysis
                                .push(format!("Failed to analyze boundary policy: {}", e));
                        }
                    }
                    ...
                    Ok(DiagnosticResult::warning(
                        ...
                        format!(
                            "Permissions boundary may impact SSM access: {}",
                            boundary_restrictions.join(", ")
                        ),
                        start_time.elapsed(),
                        severity,
                    )
    ...
    async fn verify_ec2_required_permissions(
        ...
        let required_ec2_permissions = vec![
            (
                ...
                "Required for instance status checks",
            ),
            ("ec2:DescribeInstances", "Required for instance metadata"),
            (
                "ec2:DescribeInstanceAttribute",
                "Required for instance attributes",
            ),
...
mod tests {
    ...
    async fn test_iam_diagnostics_creation() {
        ...
        match DefaultIamDiagnostics::with_default_aws().await {
            ...
            Err(e) => {
                println!(
                    "IAM diagnostics creation failed (expected in test environment): {}",
                    e
                );
            }
        }
