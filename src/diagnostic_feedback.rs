...
use ratatui::{
    ...
    text::{Span, Line},
    widgets::{Block, Borders, Clear, Gauge, List, ListItem, Paragraph, Table, Row, Cell, LineGauge},
    Frame, Terminal,
};
...
impl DiagnosticFeedbackSystem {
    ...
    pub fn update_progress(&self, progress: DiagnosticProgress) -> anyhow::Result<()> {
        debug!("Updating diagnostic progress: {}/{}", progress.completed, progress.total);
        ...
        let _ = self.event_sender.send(FeedbackEvent::UpdateProgress(progress));
    ...
    pub fn add_result(&self, result: DiagnosticResult) -> anyhow::Result<()> {
        debug!("Adding diagnostic result: {} - {:?}", result.item_name, result.status);
        
        // Check for critical issues
        if result.severity == Severity::Critical {
            warn!("Critical issue detected: {}", result.message);
            let _ = self.event_sender.send(FeedbackEvent::CriticalIssueDetected(result.clone()));
        }
    ...
    pub fn request_confirmation(&self, issue: DiagnosticResult, message: String, options: Vec<String>) -> anyhow::Result<()> {
        info!("Requesting user confirmation for critical issue: {}", issue.item_name);
        ...
        let _ = self.event_sender.send(FeedbackEvent::RequestConfirmation(confirmation));
    ...
    async fn handle_event(&mut self, event: FeedbackEvent, config: &FeedbackDisplayConfig) -> anyhow::Result<()> {
        match event {
        ...
        if let Some(terminal) = &mut self.terminal {
            ...
            terminal.draw(move |f| {
                ...
                let results_text = if state_clone.diagnostic_results.is_empty() {
                    ...
                    state_clone.diagnostic_results.iter()
...
mod tests {
    ...
    async fn test_progress_update() {
        ...
        system.update_progress(progress.clone()).expect("Failed to update progress");
    ...
    async fn test_result_addition() {
        ...
        system.add_result(result.clone()).expect("Failed to add result");
    ...
    async fn test_critical_issue_handling() {
        ...
        system.add_result(critical_result.clone()).expect("Failed to add critical result");
...
fn render_main_layout(f: &mut Frame, state: &FeedbackState, config: &FeedbackDisplayConfig, start_time: Instant) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .margin(1)
        .constraints([
...
fn render_main_content(f: &mut Frame, area: Rect, state: &FeedbackState, config: &FeedbackDisplayConfig) {
...
fn render_detailed_results(f: &mut Frame, area: Rect, state: &FeedbackState) {
    let items: Vec<ListItem> = state.diagnostic_results
        .iter()
...
fn render_confirmation_dialog(f: &mut Frame, confirmation: &ConfirmationRequest) {
    ...
    let message_text = format!(
        "項目: {}\n問題: {}\n\n{}",
        confirmation.issue.item_name,
        confirmation.issue.message,
