...
fn get_credentials_from_cli(profile: &str) -> Result<ExportedCredentials> {
    ...
    serde_json::from_slice(&output.stdout).context("Failed to parse credentials from AWS CLI")
}
...
impl AwsManager {
    ...
    pub async fn new(region: Option<String>, profile: Option<String>) -> Result<Self> {
        info!(
            "Initializing AWS manager with region: {:?}, profile: {:?}",
            region, profile
        );
    ...
    pub async fn get_session_status(&self, session_id: &str) -> Result<SsmSessionStatus> {
        ...
        let response = self
            .ssm_client
            .describe_sessions()
            .set_filters(Some(vec![aws_sdk_ssm::types::SessionFilter::builder()
        ...
        if let Some(session) = sessions.first() {
            let status = match session.status {
                ...
                Some(aws_sdk_ssm::types::SessionStatus::Disconnected) => {
                    SsmSessionStatus::Disconnected
                }
                ...
                Some(aws_sdk_ssm::types::SessionStatus::Terminating) => {
                    SsmSessionStatus::Terminating
                }
    ...
    pub async fn list_active_sessions(&self) -> Result<Vec<SsmSession>> {
        ...
        for session in session_list {
            if let (Some(session_id), Some(target)) = (&session.session_id, &session.target) {
                let status = match &session.status {
                    Some(aws_sdk_ssm::types::SessionStatus::Connected) => {
                        SsmSessionStatus::Connected
                    }
                    Some(aws_sdk_ssm::types::SessionStatus::Connecting) => {
                        SsmSessionStatus::Connecting
                    }
    ...
    async fn test_aws_manager_creation() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                println!(
                    "AWS manager creation failed (expected in test environment): {}",
                    e
                );
            }
        }
