...
pub trait AutoReconnector {
    ...
    async fn schedule_preemptive_reconnect(
        &mut self,
        session_id: &str,
        delay: Duration,
    ) -> Result<()>;
...
impl DefaultAutoReconnector {
    ...
    async fn create_replacement_session(&self, original_session: &Session) -> Result<Session> {
        ...
        info!(
            "Created replacement session {} for original session {}",
    ...
    async fn check_session_health(&self, session_id: &str) -> Result<bool> {
        ...
        if let Some(session) = sessions.get(session_id) {
            ...
            if !self.is_port_responsive(session.local_port).await {
                debug!(
                    "Session {} port {} is not responsive",
                    session_id, session.local_port
                );
                return Ok(false);
            }
...
impl AutoReconnector for DefaultAutoReconnector {
    async fn handle_disconnection(&mut self, session_id: &str, reason: &str) -> Result<bool> {
        ...
        warn!(
            "Handling disconnection for session {}: {}",
            session_id, reason
        );
        ...
        for attempt in 1..=self.config.max_attempts {
            ...
            match self.attempt_reconnection(session_id, attempt).await {
                ...
                Err(e) => {
                    error!(
                        "Reconnection attempt {} error for session {}: {}",
        ...
        // All attempts failed
        error!(
            "All reconnection attempts failed for session: {}",
            session_id
        );
    ...
    async fn preemptive_reconnect(&mut self, session_id: &str) -> Result<bool> {
        info!(
            "Performing preemptive reconnection for session: {}",
            session_id
        );
        ...
        let original_session = match self.get_session(session_id).await {
            ...
            None => {
                warn!(
                    "Session {} not found for preemptive reconnection",
                    session_id
                );
                return Ok(false);
            }
        ...
        if !self.should_preemptive_reconnect(&original_session).await {
            debug!(
                "Preemptive reconnection not needed for session: {}",
                session_id
            );
            return Ok(false);
        }
        ...
        match self.create_replacement_session(&original_session).await {
            Ok(new_session) => {
                info!(
                    "Preemptive reconnection successful: {} -> {}",
                ...
                tokio::spawn(async move {
                    ...
                    debug!(
                        "Gracefully terminating old session: {}",
                        original_session.id
                    );
                });
            ...
            Err(e) => {
                error!(
                    "Preemptive reconnection failed for session {}: {}",
                    session_id, e
                );
                Err(e)
            }
    ...
    fn configure_policy(&mut self, policy: ReconnectionConfig) {
        ...
        debug!(
            "New policy: enabled={}, max_attempts={}, base_delay={}ms, aggressive_mode={}",
    ...
    async fn attempt_reconnection(&mut self, session_id: &str, attempt: u32) -> Result<bool> {
        info!(
            "Attempting reconnection for session {} (attempt {})",
            session_id, attempt
        );
        ...
        if self.check_session_health(session_id).await? {
            info!(
                "Session {} is actually healthy, no reconnection needed",
                session_id
            );
            return Ok(true);
        }
        ...
        match self.create_replacement_session(&original_session).await {
            Ok(mut new_session) => {
                ...
                info!(
                    "Reconnection attempt {} succeeded for session: {}",
                    attempt, session_id
                );
    ...
    async fn schedule_preemptive_reconnect(
        ...
        info!(
            "Scheduling preemptive reconnection for session {} in {:?}",
            session_id, delay
        );
        ...
        tokio::spawn(async move {
            ...
            match reconnector.preemptive_reconnect(&session_id).await {
                Ok(true) => {
                    info!(
                        "Scheduled preemptive reconnection completed for session: {}",
                        session_id
                    );
                }
                Ok(false) => {
                    debug!(
                        "Scheduled preemptive reconnection was not needed for session: {}",
                        session_id
                    );
                }
                Err(e) => {
                    error!(
                        "Scheduled preemptive reconnection failed for session {}: {}",
                        session_id, e
                    );
                }
            }
    ...
    async fn cancel_reconnection(&mut self, session_id: &str) -> Result<()> {
        ...
        if attempts.remove(session_id).is_some() {
            info!(
                "Cancelled active reconnection attempt for session: {}",
                session_id
            );
        } else {
            debug!(
                "No active reconnection attempt found for session: {}",
                session_id
            );
        }
