...
pub struct ReconnectionConfig {
    ...
    #[serde(default = "default_aggressive_attempts")]
    pub aggressive_attempts: u32,

    /// Interval between aggressive attempts in milliseconds
    #[serde(default = "default_aggressive_interval_ms")]
    pub aggressive_interval_ms: u64,
}

fn default_aggressive_attempts() -> u32 {
    10
}

fn default_aggressive_interval_ms() -> u64 {
    500
}
...
impl Config {
    fn legacy_config_paths() -> Result<Vec<PathBuf>> {
        // On macOS, `dirs::config_dir()` points to "~/Library/Application Support".
        // We keep this as a legacy location for backward compatibility.
        // On Linux it typically matches ~/.config so it will be deduped.
    ...
    fn candidate_config_paths(config_path: Option<&str>) -> Result<Vec<PathBuf>> {
        if let Some(path) = config_path {
            return Ok(vec![PathBuf::from(path)]);
        }

        let preferred_json = Self::default_config_path()?;
        let preferred_toml = preferred_json.with_extension("toml");

        let mut candidates = Vec::new();
        candidates.push(preferred_json.clone());
        candidates.push(preferred_toml);

        for legacy in Self::legacy_config_paths()? {
            if legacy != preferred_json {
    ...
    async fn load_from_file(config_file: &PathBuf) -> Result<Self> {
    ...
    pub async fn load(config_path: Option<&str>) -> Result<Self> {
        let candidates = Self::candidate_config_paths(config_path)?;
        ...
        for candidate in &candidates {
            ...
            match Self::load_from_file(candidate).await {
                Ok(cfg) => {
                    loaded = Some(cfg);
                    break;
                }
        ...
        let mut config = match loaded {
            Some(cfg) => cfg,
            None => {
                // Create default configuration at the preferred location.
                ...
                if !last_errors.is_empty() {
                    // If there were parse errors in existing configs, surface them clearly.
                    let details = last_errors.join("\n  - ");
                    tracing::warn!(
                        "No valid config found. Existing config files failed to parse:\n  - {}",
                        details
                    );
                }

                let cfg = Self::default();
                cfg.save(&preferred).await?;
                cfg
            }
        };
    ...
    pub fn default_config_path() -> Result<PathBuf> {
        ...
        // with existing docs and common CLI tool conventions. Fall back to the OS
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.resources.max_memory_mb > 1024 {
            tracing::warn!(
                "max_memory_mb is set to {}MB, which is higher than recommended (10MB)",
        ...
        if self.resources.max_cpu_percent > 5.0 {
            tracing::warn!(
                "max_cpu_percent is set to {}%, which is higher than recommended (0.5%)",
        ...
        if self.session.max_sessions_per_instance > 10 {
            tracing::warn!(
                "max_sessions_per_instance is set to {}, which may cause resource issues",
                self.session.max_sessions_per_instance
            );
        }
        ...
        if self.session.health_check_interval > 60 {
            tracing::warn!(
                "health_check_interval is set to {}s, which may delay disconnect detection",
                self.session.health_check_interval
            );
        }

        if self.session.inactive_timeout < 10 {
            tracing::warn!(
                "inactive_timeout is set to {}s, which may cause premature session termination",
                self.session.inactive_timeout
            );
        }
        ...
        if self.session.reconnection.enabled {
            ...
            if self.session.reconnection.max_attempts > 20 {
                tracing::warn!(
                    "max_attempts is set to {}, which may cause excessive retry attempts",
                    self.session.reconnection.max_attempts
                );
            }
            ...
            if self.session.reconnection.base_delay_ms > 10000 {
                tracing::warn!(
                    "base_delay_ms is set to {}ms, which may cause slow reconnection",
                    self.session.reconnection.base_delay_ms
                );
            }
            ...
            if self.session.reconnection.aggressive_mode {
                ...
                if self.session.reconnection.aggressive_interval_ms < 100 {
                    tracing::warn!(
                        "aggressive_interval_ms is set to {}ms, which may cause excessive load",
                        self.session.reconnection.aggressive_interval_ms
                    );
                }
            }
        }
        ...
        if self.ui.update_interval_ms < 100 {
            tracing::warn!(
                "update_interval_ms is set to {}ms, which may cause high CPU usage",
        ...
        if self.aws.connection_timeout > self.aws.request_timeout {
            tracing::warn!(
                "connection_timeout ({}s) is greater than request_timeout ({}s)",
                self.aws.connection_timeout,
                self.aws.request_timeout
            );
        }
    ...
    pub fn print_summary(&self) {
        ...
        tracing::info!(
            ...
            self.session.reconnection.enabled
        );
