...
impl ResourceMonitor {
    ...
    pub async fn get_current_usage(&self) -> Result<ResourceUsage> {
        ...
        debug!(
            "Current resource usage: memory={:.2}MB, cpu={:.2}%, processes={}",
    ...
    pub async fn check_limits(&self) -> Result<Vec<ResourceViolation>> {
        ...
        if !violations.is_empty() {
            warn!(
                "Resource limit violations detected: {} violations",
                violations.len()
            );
        }
    ...
    pub async fn enable_low_power_mode(&mut self) -> Result<()> {
        ...
        info!(
            "Low power mode enabled: monitoring interval increased to {}s",
    ...
    pub async fn optimize_resources(&mut self) -> Result<OptimizationResult> {
        ...
        info!(
            "Starting resource optimization - current usage: {:.2}MB memory, {:.2}% CPU",
    ...
    async fn periodic_cleanup(&mut self) -> Result<()> {
        ...
        if self.cpu_samples.len() > MAX_SAMPLES {
            self.cpu_samples
                .drain(0..self.cpu_samples.len() - MAX_SAMPLES);
        }

        if self.memory_samples.len() > MAX_SAMPLES {
            self.memory_samples
                .drain(0..self.memory_samples.len() - MAX_SAMPLES);
        }
    ...
    pub async fn get_efficiency_metrics(&self) -> Result<EfficiencyMetrics> {
        ...
        debug!(
            "Efficiency metrics: memory={:.1}%, cpu={:.1}%, uptime={}s",
    ...
    pub async fn is_operating_optimally(&self) -> Result<bool> {
        ...
        debug!(
            "Operating optimally: {} (memory: {:.2}MB <= {:.2}MB, cpu: {:.2}% <= {:.2}%)",
            is_optimal,
            usage.memory_mb,
            optimal_memory_threshold,
            usage.cpu_percent,
            optimal_cpu_threshold
        );
