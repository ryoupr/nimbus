...
pub trait PerformanceMonitorTrait: Send + Sync {
    ...
    fn select_optimal_route(
        &self,
        region: &str,
    ) -> impl std::future::Future<Output = Result<Option<ConnectionRoute>>> + Send;

    /// Optimize connection based on performance data
    fn optimize_connection(
        &self,
        session_id: &str,
    ...
    fn get_session_stats(
        &self,
        session_id: &str,
    ) -> impl std::future::Future<Output = Option<SessionStats>> + Send;
...
impl PerformanceMonitor {
    ...
    pub fn configure_thresholds(&mut self, latency_ms: f64, connection_time_ms: f64) {
        ...
        info!(
            "Performance thresholds updated: latency={}ms, connection_time={}ms",
    ...
    pub async fn add_routes(&self, region: &str, routes: Vec<ConnectionRoute>) -> Result<()> {
        ...
        info!(
            "Added {} routes for region: {}",
            routes_map.get(region).map(|r| r.len()).unwrap_or(0),
            region
        );
        Ok(())
    }
    ...
    pub async fn record_connection_time(&self, session_id: &str, duration: f64) -> Result<()> {
        ...
        info!(
            "Recording connection time for session {}: {:.2}ms",
            session_id, duration
        );

        let metrics =
            PerformanceMetrics::new(session_id.to_string()).with_connection_time(duration);
        ...
        if duration > self.connection_time_threshold_ms {
            warn!(
                "Slow connection detected for session {}: {:.2}ms (threshold: {:.2}ms)",
                session_id, duration, self.connection_time_threshold_ms
            );
            self.log_performance_degradation(session_id, "slow_connection", duration)
                .await?;
        }
    ...
    pub async fn measure_latency(&self, session_id: &str, endpoint: Option<&str>) -> Result<f64> {
        ...
        if latency > self.latency_threshold_ms {
            warn!(
                "High latency detected for session {}: {:.2}ms (threshold: {:.2}ms)",
            ...
            if let Err(e) = self.optimize_connection(session_id).await {
                error!(
                    "Failed to optimize connection for session {}: {}",
                    session_id, e
                );
            }

            // Log performance degradation (要件 4.4)
            self.log_performance_degradation(session_id, "high_latency", latency)
                .await?;
        }

        // Record latency metrics
        let metrics = PerformanceMetrics::new(session_id.to_string()).with_latency(latency);
        ...
        info!(
            "Measured latency for session {}: {:.2}ms",
            session_id, latency
        );
        Ok(latency)
    }
    ...
    pub async fn select_optimal_route(&self, region: &str) -> Result<Option<ConnectionRoute>> {
        ...
        if let Some(ref route) = optimal_route {
            info!(
                "Selected optimal route for region {}: {} (score: {:.2})",
                region,
                route.endpoint,
                route.score()
            );
        ...
        if let Some(metrics) = session_metrics {
            if let Some(latest_metric) = metrics.last() {
                if let Some(route_info) = &latest_metric.route_info {
                    ...
                    if let Some(optimal_route) =
                        self.select_optimal_route(&route_info.region).await?
                    {
                        if optimal_route.endpoint != route_info.endpoint {
                            info!(
                                "Found better route for session {}: {} -> {}",
                                session_id, route_info.endpoint, optimal_route.endpoint
                            );

                            // Record optimization attempt
                            let optimization_metrics =
                                PerformanceMetrics::new(session_id.to_string())
    ...
    async fn log_performance_degradation(
        &self,
        session_id: &str,
        ...
        let detailed_info = if let Some(metrics) = session_metrics {
            let recent_metrics: Vec<_> = metrics.iter().rev().take(5).collect();
            ...
            format!(
                "Session: {}, Type: {}, Value: {:.2}, No historical data",
    ...
    pub async fn get_session_stats(&self, session_id: &str) -> Option<SessionStats> {
        ...
        let avg_connection_time =
            metrics.iter().map(|m| m.connection_time).sum::<f64>() / metrics.len() as f64;

        let avg_latency = metrics.iter().map(|m| m.latency).sum::<f64>() / metrics.len() as f64;

        let max_latency = metrics.iter().map(|m| m.latency).fold(0.0, f64::max);
        ...
        let optimizations_applied = metrics.iter().filter(|m| m.optimization_applied).count();

        let degradation_count = metrics
            .iter()
        ...
        Some(SessionStats {
            ...
            avg_latency,
            max_latency,
            min_latency: if min_latency.is_infinite() {
                0.0
            } else {
                min_latency
            },
    ...
    pub async fn generate_report(&self) -> PerformanceReport {
        ...
        for metrics in metrics_map.values() {
            if !metrics.is_empty() {
                ...
                let avg_lat = metrics
                    ...
                    .filter(|&l| l > 0.0)
                    .sum::<f64>();
                let lat_count = metrics.iter().filter(|m| m.latency > 0.0).count();
                if lat_count > 0 {
                    total_latency += avg_lat / lat_count as f64;
                }

                let session_max_lat = metrics.iter().map(|m| m.latency).fold(0.0, f64::max);
                max_latency = max_latency.max(session_max_lat);

                let session_min_lat = metrics
                    .iter()
                    .map(|m| m.latency)
                    .filter(|&l| l > 0.0)
                    .fold(f64::INFINITY, f64::min);
                ...
                total_optimizations += metrics.iter().filter(|m| m.optimization_applied).count();

                total_degradations += metrics
                    .iter()
                    .filter(|m| {
                        m.is_degraded(self.latency_threshold_ms, self.connection_time_threshold_ms)
                    })
                    .count();
            }
        }
        ...
        PerformanceReport {
            total_sessions,
            total_measurements,
            avg_connection_time: if total_sessions > 0 {
                total_connection_time / total_sessions as f64
            } else {
                0.0
            },
            avg_latency: if total_sessions > 0 {
                total_latency / total_sessions as f64
            } else {
                0.0
            },
            max_latency,
    ...
    pub async fn update_route_performance(
        ...
        if let Some(routes) = routes_map.get_mut(region) {
            if let Some(route) = routes.iter_mut().find(|r| r.endpoint == endpoint) {
                ...
                debug!(
                    "Updated route performance for {}: latency={:.2}ms, success={}, score={:.2}",
                    endpoint,
                    latency,
                    success,
                    route.score()
