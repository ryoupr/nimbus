...
impl<M: SessionManager + Send + Sync, Mon: SessionMonitor + Send + Sync> MultiSessionUi<M, Mon> {
    ...
    pub async fn render(&mut self, f: &mut Frame<'_>) -> Result<()> {
        ...
        match self.ui_state.selected_tab {
            ...
            1 => {
                self.render_resource_monitoring(f, chunks[1], &state)
                    .await?
            }
    ...
    fn render_header(&self, f: &mut Frame<'_>, area: Rect, _state: &MultiSessionState) {
        ...
        let tabs = Tabs::new(tab_titles)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title("Nimbus - Multi-Session Manager"),
            )
    ...
    async fn render_sessions_overview(
        &self,
        f: &mut Frame<'_>,
        area: Rect,
        ...
        let rows: Vec<Row> = sessions
            ...
            .map(|(i, session)| {
                ...
                Row::new(vec![
                    ...
                    Cell::from(session.priority.to_string())
    ...
    fn render_statistics_panel(&self, f: &mut Frame<'_>, area: Rect, state: &MultiSessionState) {
        ...
        let summary_text = vec![
            Line::from(vec![
                ...
                Span::styled(
                    state.total_sessions.to_string(),
                    Style::default()
                        .fg(Color::Green)
                        .add_modifier(Modifier::BOLD),
                ),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    state.active_sessions.to_string(),
                    Style::default()
                        .fg(Color::Green)
                        .add_modifier(Modifier::BOLD),
                ),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!("{:.1} MB", state.resource_usage.memory_mb),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!("{:.1}%", state.resource_usage.cpu_percent),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    state.resource_warnings.len().to_string(),
    ...
    async fn render_resource_monitoring(
        &self,
        f: &mut Frame<'_>,
        area: Rect,
        state: &MultiSessionState,
        ...
        let memory_gauge = Gauge::default()
            ...
            .label(format!(
                "{:.1} MB / 10.0 MB",
                state.resource_usage.memory_mb
            ));
    ...
    fn get_optimization_suggestions(&self, state: &MultiSessionState) -> Text<'_> {
        ...
        if state.resource_usage.memory_mb > 8.0 {
            suggestions.push(Line::from(vec![
                ...
                Span::styled(
                    "High memory usage detected. Consider terminating idle sessions.",
        ...
        if state.resource_usage.cpu_percent > 0.3 {
            suggestions.push(Line::from(vec![
                ...
                Span::styled(
                    "High CPU usage. Reduce monitoring frequency for idle sessions.",
                    Style::default().fg(Color::White),
                ),
            ]));
        }

        let total_low_priority = state
            .sessions_by_priority
            .get(&SessionPriority::Low)
            .unwrap_or(&0);
        if *total_low_priority > 2 {
            suggestions.push(Line::from(vec![
                Span::styled("💡 ", Style::default().fg(Color::Blue)),
                Span::styled(
                    "Multiple low-priority sessions detected. Consider consolidation.",
                    Style::default().fg(Color::White),
                ),
            ]));
        }
    ...
    async fn render_warnings_alerts(
        &self,
        f: &mut Frame<'_>,
        area: Rect,
        _state: &MultiSessionState,
    ) -> Result<()> {
            ...
            .map(|warning| {
                ...
                ListItem::new(vec![
                    ...
                    Line::from(vec![Span::styled(
                        format!(
                            "    Resource: {} | Current: {:.1} | Threshold: {:.1}",
        ...
        let warnings_list = List::new(warning_items).block(
            Block::default()
                .borders(Borders::ALL)
                .title(format!("Warnings & Alerts ({})", warnings.len())),
        );
    ...
    async fn render_session_details(
        &self,
        f: &mut Frame<'_>,
        area: Rect,
        _state: &MultiSessionState,
        ...
        if let Some(session) = sessions.get(self.ui_state.selected_session) {
            let details_text = vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(
                        session.connection_count.to_string(),
                        Style::default().fg(Color::White),
                    ),
                ]),
                Line::from(vec![
                    ...
                    Span::styled(
                        format!("{} bytes", session.data_transferred),
                        Style::default().fg(Color::White),
                    ),
                ]),
            ];
