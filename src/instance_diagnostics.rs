...
pub trait InstanceDiagnostics {
    ...
    async fn check_instance_type_compatibility(
        &self,
        instance_id: &str,
...
impl DefaultInstanceDiagnostics {
    ...
    fn is_ssm_compatible_instance_type(instance_type: &str) -> bool {
        ...
        !incompatible_types
            .iter()
            .any(|&incompatible| instance_type.starts_with(incompatible))
    }
...
impl InstanceDiagnostics for DefaultInstanceDiagnostics {
        ...
        match self.get_enhanced_instance_info(instance_id).await {
            Ok(Some(info)) => {
                ...
                if let Some(cpu_options) = &info.cpu_options {
                    ...
                    resource_details.insert(
                        "cpu_options".to_string(),
                        serde_json::to_value(cpu_options).unwrap(),
                    );
                }
                ...
                if let Some(storage_info) = &info.storage_info {
                    let total_storage: i32 = storage_info
                        .block_device_mappings
                        .iter()
                    ...
                    resource_details.insert(
                        ...
                        serde_json::to_value(storage_info).unwrap(),
                    );
                }
                ...
                resource_details.insert(
                    ...
                    serde_json::Value::String(info.instance_type.unwrap_or_default()),
                );
                resource_details.insert(
                    "architecture".to_string(),
                    serde_json::Value::String(info.architecture.unwrap_or_default()),
                );
                resource_details.insert(
                    "virtualization_type".to_string(),
                    serde_json::Value::String(info.virtualization_type.unwrap_or_default()),
                );
                resource_details.insert(
                    "hypervisor".to_string(),
                    serde_json::Value::String(info.hypervisor.unwrap_or_default()),
                );
            ...
            Ok(Some(info)) => {
                if let Some(ref instance_type) = info.instance_type {
                    ...
                    Ok(DiagnosticResult::warning(
                        "instance_type_compatibility".to_string(),
                        format!(
                            "Instance type information not available for {}",
                            instance_id
                        ),
                        start_time.elapsed(),
                        Severity::Low,
    ...
    async fn run_instance_diagnostics(&self, instance_id: &str) -> Result<Vec<DiagnosticResult>> {
        info!(
            "Running comprehensive instance diagnostics: {}",
            instance_id
        );
        let start_time = Instant::now();
        ...
        info!(
            "Instance diagnostics completed in {:?}",
            start_time.elapsed()
        );
        Ok(results)
    }
...
mod tests {
    ...
    fn test_platform_identification() {
        ...
        assert_eq!(PlatformInfo::from(Some("windows")), PlatformInfo::Windows);

        assert_eq!(PlatformInfo::from(Some("linux")), PlatformInfo::Linux);

        assert_eq!(PlatformInfo::from(None::<&str>), PlatformInfo::Linux);
    }

    #[test]
    fn test_ssm_compatibility_check() {
        // Modern instance types should be compatible
        assert!(DefaultInstanceDiagnostics::is_ssm_compatible_instance_type(
            "t3.micro"
        ));
        assert!(DefaultInstanceDiagnostics::is_ssm_compatible_instance_type(
            "m5.large"
