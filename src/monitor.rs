...
impl DefaultSessionMonitor {
    ...
    async fn check_network_activity(&self, session: &Session) -> Result<NetworkActivity> {
        ...
        debug!(
            "Network activity for session {}: sent={}, received={}, connections={}, activity={}",
            session.id,
            total_bytes_sent,
    ...
    async fn perform_heartbeat_check(&self, session: &Session) -> Result<(bool, SessionHealth)> {
        debug!(
            "Performing comprehensive heartbeat for session: {}",
            session.id
        );
    ...
    async fn monitor_session_health(&self, session_id: String) {
        info!(
            "Starting enhanced health monitoring for session: {}",
            session_id
        );
...
impl SessionMonitor for DefaultSessionMonitor {
    ...
    fn register_callback(&mut self, callback: Box<dyn Fn(SessionEvent) + Send + Sync>) {
        ...
        debug!(
            "Registered new callback, total callbacks: {}",
            self.callbacks.len()
        );
    }
...
impl SimpleMonitor {
    async fn monitor_session_health(&self, session_id: String) {
        info!(
            "Starting background health monitoring for session: {}",
            session_id
        );
        ...
        if let Some(session) = sessions.get(session_id) {
            ...
            if let Some(pid) = session.process_id {
                ...
                if system.process(Pid::from(pid as usize)).is_none() {
                    warn!(
                        "Process {} for session {} is no longer running",
                        pid, session_id
                    );
                }
            }
