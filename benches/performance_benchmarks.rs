...
use nimbus::health::{DefaultHealthChecker, HealthChecker};
use nimbus::manager::{DefaultSessionManager, SessionManager};
use nimbus::persistence::{PersistenceManager, PersistentSession, SqlitePersistenceManager};
use nimbus::session::{SessionConfig, SessionStatus};
use nimbus::*;
...
fn bench_session_manager(c: &mut Criterion) {
    ...
    group.bench_function("find_existing_sessions", |b| {
        b.iter(|| {
            rt.block_on(async {
                let session_manager = DefaultSessionManager::new(3)
                    .await
                    .expect("Failed to create session manager");
                session_manager
                    .find_existing_sessions(black_box("i-test123456789"), black_box(8080))
                    .await
                    .unwrap()
            })
        });
    ...
    group.bench_function("save_session", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                persistence
                    .save_session(black_box(&test_session))
                    .await
                    .unwrap()
            })
        });
...
fn bench_config(c: &mut Criterion) {
    ...
    group.bench_function("load_default_config", |b| {
        b.iter(|| rt.block_on(async { config::Config::load(black_box(None)).await.unwrap() }));
    });
            ...
            |b, &session_count| {
                b.iter(|| {
                    rt.block_on(async move {
                        let session_manager = DefaultSessionManager::new(session_count)
                            .await
                            .expect("Failed to create session manager");
    ...
    group.bench_function("initialize_all_components", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let _session_manager = DefaultSessionManager::new(3)
                    .await
                    .expect("Failed to create session manager");
